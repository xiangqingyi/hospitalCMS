...
exports.list = async function (req, res) {
    ...
    return res.render('server/patient/list', {
        title: '患者列表',
        patients: arr,
        pageInfo: pageInfo,
        Menu: 'list'
...
exports.one = async function (req, res) {
    ...
    const results = await User.findById(id).populate('author').populate('roles')
    return res.render('server/patient/item', {
        patient: results
    })
}
...
exports.add = async function (req, res) {
  let method = req.method;
  if (method === 'GET') {
    res.render('server/patient/add', {
      Menu: 'add'
    });
  } else if (method === 'POST') {
    //let obj = req.body;
    let obj = _.pick(req.body,  'caseReport','username', 'password', 'email', 'mobile', 'name', 'avatar', 'gender', 'birthday', 'description', 'address', 'position', 'questions', 'tel');
    console.log(obj);
...
//   } 
    ...
    Role.findOne({ name: "Patient" },async  function (err, role) {
      console.log('role', role);
...
exports.edit = async function (req, res) {
    let id = req.params.id
    const patient = await User.findById(id).populate('author')
    const isAuthor = patient.author && ((patient.author._id + '') === req.session.user._id)
    const isAdmin = req.isAdmin
...
exports.del = async function (req, res) {
    ...
    const patient = await User.findById(id).populate('roles').populate('author')
    if (!patient) {
        return res.render('server/info', {
            message: "不存在这个医生"
        })
