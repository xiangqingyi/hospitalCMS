...
let ward = require('../../controllers/server/ward')

router.use(function (req, res, next) {
    // console.log('user: ' + Date.now());
    res.locals.Path = 'ward';
    next();
  });
...
router.route('/').get(action.checkAction('USER_INDEX'), ward.list);

router.route('/add').all(action.checkAction('USER_CREATE'), ward.add);
...
module.exports = function (app) {
  let path = util.translateAdminDir('/ward');
  app.use(path, router);
};
